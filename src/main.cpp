...
// Plugin metadata (F4SEPlugin_Version) is now generated by the
// commonlibf4.plugin xmake rule from xmake.lua's target name/author/
// description + set_version(), declaring UsesAddressLibrary(NG) and
// IsLayoutDependent(NG) instead of an explicit CompatibleVersions list — that
// is what lets F4SE accept the DLL on OG, NG and AE alike instead of only the
// runtimes we happened to enumerate. See docs/MultiRuntime.md.
//
// We read engine struct offsets and vtable slots (e.g. Actor::race, vtable
// index 0x86 for Load3D), so F4SEPlugin_Query is still the real gate: reject
// anything below 1.10.163 (OG), which predates all three layouts this DLL
// understands.
...
// NG/AE call Preload then Load; OG only ever calls Load. Do the setup once
// no matter which entry points the runtime uses.
...
// OG has no Preload; it only ever calls Load.
...
// NG/AE call Preload, early enough to patch before the game touches things.
